#siamese transformer
#将重建目标换成了关节坐标，x_masked + x_unmasked作为孪生encoder的输入
...
#定义一个新的attention机制，用于decoder中。其中q来自孪生网络中的一个encoder的输出（masked），k和v来自于另一个encoder的输出（未经mask）
class Decoder_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # Initialize the linear layer to map input to queries, keys, values
        self.q = nn.Linear(dim, dim, bias=qkv_bias)#dim * 3改为dim
    ...
    def forward(self, x, k, v):
        ...
        k = self.k(k).reshape(B, N_k, self.num_heads, C_k // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(v).reshape(B, N_v, self.num_heads, C_v // self.num_heads).permute(0, 2, 1, 3)
...
class Block(nn.Module):#用于encoder中的block
...
class DecoderBlock(nn.Module):#新建一个专门用于Decoder的block
    # This is the constructor of a class responsible for initializing instances of the class
    # dim: The dimension of the input features. 
    # mlp_ratio controls the size of the hidden layers in the MLP relative to the input feature dimension
    # mlp_out_ratio controls the size of the output layer in the MLP relative to the input feature dimension.
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_out_ratio=1.,
        ...
        self.attn = Attention(dim, num_heads=num_heads,
                              qkv_bias=qkv_bias, qk_scale=qk_scale,
                              attn_drop=attn_drop, proj_drop=drop)
        self.multihead_attn = Decoder_Attention(dim, num_heads=num_heads,
                              qkv_bias=qkv_bias, qk_scale=qk_scale,
                              attn_drop=attn_drop, proj_drop=drop)
    ...
    def multihead_attn_block(self, x, x_motion):
        #q来自x，k和v来自x_motion
        x = self.multihead_attn(x, x_motion, x_motion)
        return x
    def forward(self, x, x_motion): #x是f2,x_motion是f1
        #x = x + self.drop_path(self.attn(self.norm1(x), seqlen))
        #x = x + self.drop_path(self.mlp(self.norm2(x)))
        x = x + self.drop_path(self.multihead_attn_block(self.norm1(x), x_motion))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class Transformer(nn.Module):
    ...
    def motion_aware_random_masking(self, x, x_orig, mask_ratio, tau):
        ...
        x: [NM, L, D], sequence，注：我推出的输入到该函数中的x的形状为[N, T*V, C]
        x_orig: patchified original skeleton sequence,注：我推出的输入的x_orig的形状是[N, T, V, C]
        tau: τ
        """
        NM, L, D = x.shape
        ...
        """
        也可尝试在最后一维进行复制填充或0填充，代码为：
        x_orig_motion[:, :-1, :, :] = torch.abs(x_orig[:, 1:, :, :] - x_orig[:, :-1, :, :])
        x_orig_motion[:, -1:, :, :] = x_orig_motion[:, -2:-1, :, :]#相邻帧复制填充，或0填充：x_orig_motion[:, -1:, :, :] = 0"""
        
        
        
        #计算运动强度I
        # Perform average pooling along the fourth dimension (indexed as 3),
        # i.e., take the average along the coordinate channel dimension for each joint, resulting in a new value for that joint. 
        # This transforms the original dimension (NM, TP, VP, C) into (NM, TP, VP).
        x_orig_motion = x_orig_motion.mean(dim=[3])  # NM, TP, VP
        x_orig_motion = x_orig_motion.reshape(NM, L)
        
        """#改动版
        #计算运动强度I（将沿着每个关节点的坐标通道求均值，改为求每个关节点所代表的位移向量的模长）
        # Perform 求模长 along the fourth dimension (indexed as 3),
        # i.e., take the 模长 along the coordinate channel dimension for each joint, resulting in a new value for that joint. 
        # This transforms the original dimension (NM, TP, VP, C) into (NM, TP, VP).
        x_orig_motion = x_orig_motion.pow(2)  # 计算每个坐标值的平方
        x_orig_motion = x_orig_motion.sum(dim=3)#沿着C通道求（平方）和  #NM, TP, VP
        x_orig_motion = x_orig_motion.sqrt()#求平方和的算术平方根
        x_orig_motion = x_orig_motion.reshape(NM, L)"""
        
        
        # π == Softmax(I/τ )
        x_orig_motion = x_orig_motion / (torch.max(x_orig_motion, dim=-1, keepdim=True).values * tau + 1e-10)
        x_orig_motion_prob = F.softmax(x_orig_motion, dim=-1) # π

        noise = torch.log(x_orig_motion_prob) - torch.log(-torch.log(torch.rand(NM, L, device=x.device) + 1e-10) + 1e-10)  # Gumbel-max trick

        # noise = torch.rand(N, L, device=x.device)  # noise in [0, 1]

        # sort noise for each sample
        ids_shuffle = torch.argsort( #返回张量中元素值（从小到大）排序后的索引
            noise, dim=1
        )  # ascend: small is keep, large is remove
        ids_restore = torch.argsort(ids_shuffle, dim=1)#恢复原始张量的位置索引顺序
    ...
    def forward_encoder(self, x, mask_ratio, motion_aware_tau):# 传入encoder时x形状：[NM, T, V, C] torch.Size([2, 120, 25, 3])
        x_orig = self.patchify(x) #x_orig保留嵌入之前的信息，后面mask会用到。形状[N, (T/4)*V, u*p]   #torch.Size([2, 750, 12])
        # embed skeletons
        x = self.joints_embed(x)# 经过embedding后x的形状，[N, TP, VP, dim_feat=256]  torch.Size([2, 30, 25, 256])
        ...
        if motion_aware_tau > 0:
            ...
            x, mask, ids_restore, _ = self.motion_aware_random_masking(x, x_orig, mask_ratio, motion_aware_tau)#x形状：torch.Size([2, 149, 256]). 其中149=750*mask_ratio(0.8)，表示保留的关节个数。
        else:   
    ...
    #孪生encoder中的分支，该分支不进行mask操作
    def forward_encoder_no_masking(self, x):# 传入encoder时x形状：[NM, T, V, C] torch.Size([2, 120, 25, 3])
        x_orig = self.patchify(x) #x_orig保留嵌入之前的信息，后面mask会用到。形状[N, (T/4)*V, u*p]   #torch.Size([2, 750, 12])
        # embed skeletons
        x = self.joints_embed(x)# 经过embedding后x的形状，[N, TP, VP, dim_feat=256]  torch.Size([2, 30, 25, 256])
        ...
        x = self.norm(x)#从encoder中输出的张量的形状：
 
        return x

    def forward_decoder(self, x, x_frame1, ids_restore):# x表示传入的latent，其形状：torch.Size([2, 149, 256])
        NM = x.shape[0] #2
        ...
        C = x.shape[-1]
        x_frame1 = self.decoder_embed(x_frame1)#前一帧

        # append intra mask tokens to sequence
        # self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_dim_feat))
        # the dimension of x is :(batch, len_keep, D)
        x_mask_tokens = self.mask_token.repeat(NM, TP * VP - x.shape[1], 1)#torch.Size([2, 601, 256])
        # 确认 x 和 x_mask_tokens 的维度匹配
        #print(f"xshape:{x.shape}")  # 打印 x 的维度
        #print(f"x_mask_tokens_shape:{x_mask_tokens.shape}")  # 打印 x_mask_tokens 的维度
        # 检查维度，并确保它们在拼接时具有相同的维度
        assert x.dim() == x_mask_tokens.dim(), f"维度不匹配: x.dim()={x.dim()}, x_mask_tokens.dim()={x_mask_tokens.dim()}"
        x_ = torch.cat([x[:, :, :], x_mask_tokens], dim=1)  # no cls token torch.Size([2, 750, 256])
        x_ = torch.gather(
        ...
        for idx, blk in enumerate(self.decoder_blocks):
            x = blk(x, x_frame1)  #torch.Size([2, 750, 256])
    ...
    #提取未来帧(后一帧)信息，作为孪生网络编码器的一个输入（执行随机mask），同时也是预测目标 
    def extract_frame2(self, x, motion_stride=1):
        '''
        imgs: [NM, T, V, C]
        x_cos: [NM, T, V, 1]
        '''
        ...
        x_shape = x.shape
        # 创建形状为 [NM, T, V] 的零张量
        x_frame2 = torch.zeros_like(x, dtype=x.dtype, device=x.device)
        
        # 未来帧
        x_frame2[:, :-motion_stride :] = x[:, motion_stride:, :] #形状[N, T-1, V, C]
        x_frame2[:, -motion_stride:, :] = 0
        return x_frame2
       
    """#重建帧内关节点之间的余弦相似度
    def extract_cosine_values(self, x, motion_stride=1):
        '''
        imgs: [NM, T, V, C]
        cosine_values: [NM, T, V, 1]
        '''
        # generate motion
        
        # Creating a zero tensor cosine_values with the same shape as the input x to store the extracted motion information.
        #cosine_values = torch.zeros_like(x)
        
        # 获取张量 x 的形状
        x_shape = x.shape
        # 创建形状为 [NM, T, V] 的零张量
        cosine_values = torch.zeros(x_shape[:-1], dtype=x.dtype, device=x.device)
        ...
        # 计算余弦相似度
        cosine_values[:, :, :-motion_stride] = F.cosine_similarity(u, v, dim=-1) #F.cosine_similarity(u, v, dim=-1)的形状是[NM, T, V-1]
        cosine_values[:, :, -motion_stride:] = 0
        # 添加一个维度，保持张量的维度不变
        cosine_values = cosine_values.unsqueeze(-1) # [NM, T, V, 1]
        
        return cosine_values"""
    
    """#重建原关节
    def extract_joint(self, x):
        '''
        imgs: [NM, T, V, 3]
        '''
        # generate motion
    ...
    def forward_loss(self, imgs, pred, mask):#这里的imgs相当于传入的joint_coordinates,形状为#torch.Size([2, 120, 25, 1]), pred的形状：torch.Size([2, 750, 4])
        """
        imgs: [NM, T, V, 1]
        pred: [NM, TP * VP, t_patch_size * patch_size * 1]
        mask: [NM, TP * VP], 0 is keep, 1 is remove,
        """
        target = self.patchify(imgs)  # [NM, TP * VP, t_patch_size*patch_size*1]
    ...
    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, **kwargs):
        ...
        x = x.permute(0, 4, 2, 3, 1).contiguous().view(N * M, T, V, C)#torch.Size([2, 120, 25, 3])#其中一个encoder的输入信息
        #joint_coordinates = self.extract_joint(x)#torch.Size([2, 120, 25, 3])#先保存原始关节点坐标，作为真实值（目标）
        x_frame2 = self.extract_frame2(x, motion_stride)#torch.Size([2, 120, x, 3])#提取未来帧关节点坐标，作为真实值（目标）
        #孪生encoder
        
        #未mask的分支，x
        x = self.forward_encoder_no_masking(x)#[2, 750, 256]
        #mask的分支, x_frame2
        latent, mask, ids_restore = self.forward_encoder(x_frame2, mask_ratio, motion_aware_tau)#latentshape: torch.Size([2, ..., 256])   #maskshape: torch.Size([2, ...])
        #Decoder
        pred = self.forward_decoder(latent, x, ids_restore)  # [NM, TP * VP, C] torch.Size([2, ..., 12])
        loss = self.forward_loss(x_frame2, pred, mask)
