...
class Transformer(nn.Module):
    ...
    def extract_cos(self, x, motion_stride=1):
        ...
        x_shape = x.shape
        # 创建形状为 [NM, T, V] 的零张量
        x_cos = torch.zeros(x_shape[:-1], dtype=x.dtype, device=x.device)
        
        # 计算同一帧中相邻索引位置关节点之间的余弦相似度
        # 取前后两个索引位置的关节点向量
        u = x[:, :, :-motion_stride] #形状[N, T, V-1, C]
        v = x[:, :, motion_stride:] #形状[N, T, V-1, C]
        
        # 计算余弦相似度
        x_cos[:, :, :-motion_stride] = F.cosine_similarity(u, v, dim=-1) #F.cosine_similarity(u, v, dim=-1)的形状是[NM, T, V-1]
        x_cos[:, :, -motion_stride:] = 0 # 最后一个关节点的余弦相似度设置为0
        # 添加一个维度，保持张量的维度不变
        x_cos = x_cos.unsqueeze(-1) # [NM, T, V, 1]
    ...
    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, **kwargs):
        ...
        x_cos = self.extract_cos(x, motion_stride)#torch.Size([2, 120, 25, 1])#先提取余弦相似度，作为真实值（目标）
