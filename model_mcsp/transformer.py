...
class SkeleEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv2d(dim_in, dim_feat, kernel_size=kernel_size, stride=kernel_size)#骨架嵌入/投影
...
class Transformer(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio, motion_aware_tau):# x形状：[N, T, V, C]
        x_orig = self.patchify(x)# x_orig保留嵌入之前的信息，后面mask会用到。形状[N, T/4, V/1, 4*1*C]

        # embed skeletons
        x = self.joints_embed(x)# 经过embedding后 x 的形状，[N, TP, VP, dim_feat=256]  torch.Size([2, 30, 25, 256])

        NM, TP, VP, _ = x.shape# N, T, V, _
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x = x_.view([NM, TP, VP, C])# Reshape x_ into a four-dimensional tensor
        ...
        #a motion prediction head , which takes decoded features as input and predicts the temporal motion of the input skeleton sequence.
    ...
    '''def extract_motion(self, x, motion_stride=1):
        ...
        # Creating a zero tensor x_motion with the same shape as the input x to store the extracted motion information.
        x1_motion = torch.zeros_like(x)
        x2_motion = torch.zeros_like(x)
        
        
        x1_motion[:, :-motion_stride, :, :] = x[:, motion_stride:, :, :] - x[:, :-motion_stride, :, :]
        x1_motion[:, -motion_stride:, :, :] = 0
        x2_motion[:, :-motion_stride-1, :, :] = x[:, motion_stride+1:, :, :] - x[:, motion_stride:-motion_stride, :, :]
        x2_motion[:, -motion_stride*2:, :, :] = 0
        x_motion = (x2_motion - x1_motion) / (1.0/30.0)**2
        return x_motion'''
    ...
    """def extract_motion(self, x, motion_stride=1):
        ...
        x_motion[:, :-motion_stride, :, :] = x[:, motion_stride:, :, :] - x[:, :-motion_stride, :, :]#位移
        x_motion[:, -motion_stride:, :, :] = 0
        x_motion = x_motion/(1.0/30.0)#平均速度
        return x_motion"""
    ...
    """def extract_motion(self, x, motion_stride=1):
        ...
        x_motion[:, :-motion_stride, :, :] = (x[:, motion_stride:, :, :]) - x[:, :-motion_stride, :, :]
        x_motion[:, -motion_stride:, :, :] = 1.0e-6
        # 计算每个向量的长度
        epsilon = 1e-8  # 很小的常数值
        vector_lengths = torch.norm(x_motion, dim=3, keepdim=True) + epsilon

        # 将向量除以其长度
        x_motion = x_motion / vector_lengths
        x_motion[:, -motion_stride:, :, :] = 0

        return x_motion"""

    def forward_loss(self, imgs, pred, mask):#这里的imgs相当于x_motion
        """
        imgs: [NM, T, V, 3]
        pred: [NM, TP * VP, t_patch_size * patch_size * C]
        mask: [NM, TP * VP], 0 is keep, 1 is remove,
        """
        target = self.patchify(imgs)  # [NM, TP * VP, t_patch_size * patch_size * C]
    ...
    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, **kwargs):
        ...
        pred = self.forward_decoder(latent, ids_restore)  # [NM, TP * VP, t_patch_size * patch_size * dim_in]
